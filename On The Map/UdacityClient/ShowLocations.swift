...
extension UdacityClient {
    ...
    func getObjectID(_ location: String, _ link: String, _ latitude: Double, _ longitude: Double, completion: @escaping (_ result: String?, _ error: String?) -> (Void)) {
        ...
        request.httpBody = "{\"uniqueKey\": \"\(self.appDelegate.key ?? "748492837")\", \"firstName\": \"\(self.appDelegate.firstName ?? "John")\", \"lastName\": \"\(self.appDelegate.lastName ?? "Doe")\",\"mapString\": \"\(location)\", \"mediaURL\": \"\(link)\",\"latitude\": \(latitude), \"longitude\": \(longitude)}".data(using: String.Encoding.utf8)
