...
extension UdacityClient {
    ...
    func getObjectID(_ location: String, _ link: String, _ latitude: Double, _ longitude: Double, completion: @escaping (_ result: String?, _ error: String?) -> (Void)) {
        ...
        request.httpBody = "{\"uniqueKey\": \"\(Locations.sharedInstance.key ?? "748492837")\", \"firstName\": \"\(Locations.sharedInstance.firstName ?? "John")\", \"lastName\": \"\(Locations.sharedInstance.lastName ?? "Doe")\",\"mapString\": \"\(location)\", \"mediaURL\": \"\(link)\",\"latitude\": \(latitude), \"longitude\": \(longitude)}".data(using: String.Encoding.utf8)
